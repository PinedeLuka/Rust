...
fn borrow_book ( vector : &mut Vec<Livre>){
    ...
    if check == false{
        println!("le livre demandé est introuvable");
    }
}
...
fn give_back ( vector : &mut Vec<Livre>){
    let mut book = String::new(); 
    let mut check = false;
    println!("veillez saisir le livre à rendre.");
    io::stdin().read_line(&mut book).expect("Failed to read line");
    book=book.trim().to_string();
    println!("vous avez choisis '{}'", book);
    for i in vector{
